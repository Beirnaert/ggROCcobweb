...
convert_to_cobweb_df <- function(axisLim = NULL){
...
geom_cobweb_bg_proto <- ggplot2::ggproto("geom_cobweb_bg_proto", Geom,
                                         required_aes = c("AUCs"),
                                         default_aes = aes(colour = "black", alpha = 0.75, linetype = "solid", size = 1),
                                         draw_key = draw_key_abline,
                                         draw_group = function(data, panel_params, coord) {
                                             #print(data)
                                             ...
                                             bg_labels <- grid::textGrob(label = "text",
                                                                         x = coords_df$x[1],
                                                                         y = coords_df$y[1],
                                                                         just = "right")
...
geom_cobweb <- function(mapping = NULL, data = NULL, stat = 'identity',
                        ...
                        grid_lty = "dashed", grid_lwd = 1, lwd = 2, grid_alpha = 0.75, ...) {
    ...
    background_labels = data.frame(x = rep(0.5,3), y = c(0.25,0.5,0.75))
    
    list(
        ggplot2::layer(
            geom = geom_cobweb_bg_proto,
            mapping = aes(AUCs = levels,  group = level_factors),
            data = background,
            stat = stat,
            position = position,
            show.legend = FALSE,
            inherit.aes = FALSE,
            params = list(na.rm = na.rm, linetype = grid_lty, size = grid_lwd, alpha = grid_alpha, ...)
        ),
        ggplot2::layer(
